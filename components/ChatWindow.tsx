...
const ChatWindow: React.FC = () => {
  ...
  const [settings, setSettings] = useState<VoiceSettings>(() => {
    const saved = localStorage.getItem('aideia-settings-v8');
  ...
  useEffect(() => {
    localStorage.setItem('aideia-settings-v8', JSON.stringify(settings));
  }, [settings]);

  useEffect(() => {
    const saved = localStorage.getItem('aideia-conversations-v8');
    if (saved) {
  ...
  useEffect(() => {
    if (conversations.length > 0) {
      localStorage.setItem('aideia-conversations-v8', JSON.stringify(conversations));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let fullText = "";
      await generateStreamingResponse(currentInput, 'deepseek', history.concat(userMsg), (chunk) => {
        fullText += chunk;
        setConversations(prev => prev.map(c => 
          c.id === activeConversationId 
