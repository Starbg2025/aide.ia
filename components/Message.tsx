...
const Message: React.FC<MessageProps> = ({ message }) => {
  ...
  const bubbleClasses = `px-4 py-3 rounded-2xl ${isUser ? 'bg-primary-500 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 rounded-bl-none shadow-sm'}`;
  const avatar = isUser ? 
    <UserIcon className="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 p-1 text-gray-600 dark:text-gray-300"/> : 
